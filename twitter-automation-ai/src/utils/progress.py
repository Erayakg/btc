...
class Progress:
    ...
    def print_progress(self, current, finished=False, error_count=0, no_tweets_limit=False):
        # Basit bir progress çıktısı
        if self.total and not no_tweets_limit:
            percent = (current / self.total) * 100
            print(f"[Progress] {current}/{self.total} ({percent:.1f}%) tweets scraped | Errors: {error_count}")
        else:
            print(f"[Progress] {current} tweets scraped | Errors: {error_count}")

        if finished:
            print("[Progress] Scraping finished.")

...
def setup_driver():
    ...
    # options.add_argument("--headless")  # Test aşamasında kapalı dursun
...
def get_latest_gmail_code(email_address, email_password):
    try:
        ...
        with MailBox("imap.gmail.com").login(email_address, email_password) as mailbox:
            # Son 10 dakikadaki maillere bak
...
def login_and_save_cookies(username, password, email_address=None, email_password=None):
    ...
    try:
        ...
        if email_address and email_password:
            try:
                # Farklı kod input alanlarını dene
                code_input = None
                selectors = [
                    (By.NAME, "text"),
                ...
                if code_input:
                    ...
                    for attempt in range(3):  # 3x dene, 3 sn arayla
...
for line in lines:
    ...
    parts = line.split(":")
    if len(parts) == 4:
        username, password, email_address, email_password = parts
    elif len(parts) == 2:
        username, password = parts
        email_address = email_password = None
    else:
        continue
